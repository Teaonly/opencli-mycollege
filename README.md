...
这是一个验证性项目，旨在探索 **AI 在大学教师真实日常场景中的落地应用**。

## 🎯 项目目标

- **真实场景驱动**：聚焦中国大学教师（或类似高校）的日常痛点，包括教务系统、邮件系统、OA、科研管理系统、财务报销等。
- **AI + CLI 融合**：用 OpenCLI 把学校网页系统转化为可编程的 CLI 命令，再结合智能体实现自动化。
- **可验证、可复用**：提供完整的命令定义、prompt 示例、自动化脚本和工作流模板，任何教师或开发者都能快速上手并适配自己的学校系统。
- **最终愿景**：让 AI 成为大学教师的“数字分身”，把节省下来的时间还给教学、科研和学生指导。

## 安全说明

为了确保信息安全，所有学校 IT/OA 系统的 URL 均通过配置文件实现，代码中并不包含这些 URL 地址。
